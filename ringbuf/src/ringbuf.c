...
void
ringbuf_memcpy_peek(void *dst, const ringbuf_t src, size_t count)
{
    if (count > ringbuf_bytes_used(src))
        return;

    uint8_t *u8dst = dst;
    const uint8_t *bufend = ringbuf_end(src), *virttail = src->tail;
    size_t nwritten = 0;
    while (nwritten != count) {
        assert(bufend > virttail);
        size_t n = MIN(bufend - virttail, count - nwritten);
        if(dst)
        {
			memcpy(u8dst + nwritten, virttail, n);
		}
        virttail += n;
        nwritten += n;
